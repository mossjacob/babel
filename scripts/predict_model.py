...
from babel.data import SingleCellDataset, DummyDataset, PairedDataset
from babel.data.loaders import (
    TENX_PBMC_RNA_DATA_KWARGS, TENX_PBMC_ATAC_DATA_KWARGS,
    repool_genes, repool_atac_bins, liftover_atac_adata
)
from babel import model_utils, plot_utils, utils, evaluation
...
def load_rna_files_for_eval(
    ...
    rna_data_kwargs = copy.copy(TENX_PBMC_RNA_DATA_KWARGS)
    if no_filter:
        rna_data_kwargs = {
    ...
    try:
        ...
        sc_rna_full_dataset = SingleCellDataset(
            mode="skip",
            **rna_data_kwargs,
        )
        assert all(
            [x == y for x, y in zip(rna_genes, sc_rna_full_dataset.adata.var_names)]
        ), "Mismatched genes"
        _temp = sc_rna_full_dataset[0]  # Try that query works
        # adata_utils.find_marker_genes(sc_rna_full_dataset.adata, n_genes=25)
        # marker_genes = adata_utils.flatten_marker_genes(
        #     sc_rna_full_dataset.adata.uns["rank_genes_leiden"]
        # )
        marker_genes = []
        # Write out the truth
        ...
        # Update length later
        sc_rna_full_dataset = DummyDataset(
...
def load_atac_files_for_eval(
    ...
    atac_data_kwargs = copy.copy(TENX_PBMC_ATAC_DATA_KWARGS)
    ...
    try:
        sc_atac_full_dataset = SingleCellDataset(
            mode="skip",
        ...
        assert all(
            [x == y for x, y in zip(atac_bins, sc_atac_full_dataset.adata.var_names)]
        )
        ...
        sc_atac_full_dataset = DummyDataset(
...
def main():
    ...
    sc_atac_full_dataset, atac_bins = load_atac_files_for_eval(
        ...
        args.bins,
        args.liftHg19toHg38,
        sc_rna_full_dataset if hasattr(sc_rna_full_dataset, "adata") else None,
    )
    # Write out the truth
    if hasattr(sc_atac_full_dataset, "adata"):
        logging.info("Writing truth ATAC binary counts")
        sc_atac_full_dataset.adata.write_h5ad(
            os.path.join(args.outdir, "truth_atac.h5ad")
        )

    if isinstance(sc_rna_full_dataset, DummyDataset) and isinstance(
        sc_atac_full_dataset, DummyDataset
    ):
    ...
    if isinstance(sc_rna_full_dataset, DummyDataset) and not isinstance(
        sc_atac_full_dataset, DummyDataset
    ):
        sc_rna_full_dataset.length = len(sc_atac_full_dataset)
    elif isinstance(
        sc_atac_full_dataset, DummyDataset
    ) and not isinstance(sc_rna_full_dataset, DummyDataset):
    ...
    for i, ckpt in enumerate(args.checkpoint):
        ...
        if (
            isinstance(sc_rna_full_dataset, SingleCellDataset)
            and not args.skiprnasource
        ):
            if not args.transonly:
                evaluation.evaluate_rna_from_rna(
                    spliced_net,
            ...
            evaluation.evaluate_atac_from_rna(
                spliced_net,
                sc_dual_full_dataset,
                rna_genes,
                atac_bins,
        ...
        if (
            isinstance(sc_atac_full_dataset, SingleCellDataset)
            and not args.skipatacsource
        ):
            evaluation.evaluate_rna_from_atac(
                spliced_net,
                sc_dual_full_dataset,
                rna_genes,
                atac_bins,
            ...
            if not args.transonly:
                evaluation.evaluate_atac_from_atac(
                    spliced_net,
                    sc_dual_full_dataset,
                    rna_genes,
                    atac_bins,
